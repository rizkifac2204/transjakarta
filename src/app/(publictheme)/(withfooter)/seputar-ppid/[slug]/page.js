...
async function SeputarPPIDPage({ params }) {
  if (!params?.slug) notFound();

  const fullSlug = `/seputar-ppid/${params.slug}`;

  const isValid = SEPUTAR_PPID.filter((item) => !item.static).some(
  ...
  return (
    <>
      <Banner
        ...
        maplink={[
          { label: "Home", href: "/" },
          { label: "Seputar PPID", href: "/seputar-ppid" },
          { label: title },
        ]}
      />
...
export default SeputarPPIDPage;
