...
async function UkpbjSlugPage({ params }) {
  if (!params?.slug) notFound();

  const fullSlug = `/ukpbj/${params.slug}`;

  const isValid = UKPBJ.filter((item) => !item.static).some(
