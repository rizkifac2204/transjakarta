...
export async function GET(_request, { params }) {
  try {
    ...
    const data = await getUkpbjRegulasiDetailById(parsedId);
...
export async function PATCH(request, { params }) {
  ...
  try {
    ...
    const curData = await getUkpbjRegulasiDetailById(parsedId);
    if (!curData) {
      return Response.json(
    ...
    if (status_file === "delete") {
      await hapusFile(curData.file, PATH_UPLOAD.ukpbj.regulasi);
    ...
    const updated = await updateUkpbjRegulasi(parsedId, updateData);
...
export async function DELETE(_request, { params }) {
  try {
    ...
    const deleted = await deleteUkpbjRegulasi(parsedId);
