...
export function pathShelter(shelter) {
  if (!shelter) return PATH_UPLOAD.shelter;
  return PATH_UPLOAD.shelter + `/` + shelter?.id;
}
...
export async function POST(request, { params }) {
  ...
  try {
    ...
    const updatedAnswer = await prisma.shelter_survey_answer.update({
      where: {
        shelter_survey_id_question_id: {
          shelter_survey_id: parsedSurveyId,
          question_id: parsedQuestionId,
