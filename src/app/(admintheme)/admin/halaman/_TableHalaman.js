    ...
    >
      <Tooltip content="Detail" placement="top" arrow animation="shift-away">
        <Link
          ...
          href={`/admin/halaman/${encodeId(rowData.id)}`}
        >
      ...
      <Tooltip content="Edit" placement="top" arrow animation="shift-away">
        <Link
          className="action-btn"
          href={`/admin/halaman/${encodeId(rowData.id)}/edit`}
        >
...
function TableHalaman({ data = [] }) {
  ...
  const handleDelete = async (id) => {
    ...
    setDeletingRowId(id);
    try {
      const res = await axios.delete(`/api/halaman/${id}`);
  ...
  const columns = [
    ...
    COLUMNHELPER.accessor("judul", { header: "Judul" }),
    ...
    <Card
      ...
      headerslot={
        <div className="flex items-center justify-between">
          ...
          />
          <Link href="/admin/halaman/add">
            <button className="btn btn-sm btn-gold ml-2">+ Tambah</button>
          </Link>
        </div>
      }
    >
