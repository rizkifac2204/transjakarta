    ...
    >
      <Tooltip content="Edit" placement="top" arrow animation="shift-away">
        <Link
          ...
          href={`/admin/dip/${kategori}/${encodeId(rowData.id)}/edit`}
          scroll={false}
        >
...
function TableDip({ kategori, data = [] }) {
  ...
  const handleDelete = async (id) => {
    ...
    setDeletingRowId(id);
    try {
      const res = await axios.delete(`/api/dip/${kategori}/${id}`);
  ...
  const columns = [
    ...
    COLUMNHELPER.accessor(
      (row) => row.dip_header?.label || "-", // fallback null
      {
        id: "dip_header", // ID kolom (harus unik)
        header: "Header",
      }
    ),
    ...
    COLUMNHELPER.accessor("file", {
      ...
      cell: (info) => {
        ...
        return (
          <a
            href={`/api/services/file/uploads/dip/${kategori}/${url}`}
            target="_blank"
