...
function ActionFoto({ profile }) {
  ...
  async function handleDelete() {
    setIsUploading(true);
    try {
      const res = await axios.delete(`/api/profile/upload`);
      toast.success(res.data.message);
    } catch (err) {
      console.error(err);
      toast.error(
  ...
  return (
    <div className="md:h-[186px] md:w-[186px] h-[140px] w-[140px] md:ml-0 md:mr-0 ml-auto mr-auto md:mb-0 mb-4 rounded-full ring-4 ring-slate-100 relative">
      <FilePreview
        fileUrl={
          profile?.foto
            ? `/api/services/file/uploads/${PATH_UPLOAD.admin}/${profile?.foto}`
            : null
        }
        filename={profile?.foto || "Profile"}
        isUser={true}
      ...
      {profile?.foto ? (
        <>
          ...
          <Tooltip content="Ganti" placement="top" arrow animation="shift-away">
            <button
        ...
        <Tooltip content="Upload" placement="top" arrow animation="shift-away">
          <button
            className={`absolute ${isUploading && "bg-slate-800"}
            right-2 h-8 w-8 bg-slate-50 text-slate-600 rounded-full shadow-sm flex flex-col items-center justify-center md:top-[140px] top-[100px]`}
            onClick={() => uploadRef.current?.click()}
