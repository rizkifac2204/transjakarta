...
function ActionCell({ rowData, deletingRowId, handleDelete, setCurrentInfo }) {
    ...
    >
      <Tooltip content="Detail" placement="top" arrow animation="shift-away">
        <Link
          ...
          href={`/admin/pengguna/${encodeId(rowData.id)}`}
        >
      ...
      <Tooltip content="Lihat" placement="top" arrow animation="shift-away">
        <button
          className={`action-btn ml-3 hidden md:block`}
          type="button"
          onClick={() => setCurrentInfo(rowData)}
        >
          <Icon
...
function ContentUser({ data }) {
  ...
  const columns = [
    ...
    COLUMNHELPER.accessor("level.level", { header: "Role" }),
    COLUMNHELPER.accessor("telp", { header: "Telp/HP" }),
    {
      id: "actions",
      header: "Aksi",
      cell: ({ row }) => (
                      ...
                      table.getRowModel().rows.map((row) => (
                        <tr
                          ...
                          className={`hover:bg-slate-100 dark:hover:bg-slate-700 ${
                            row.original.id == currentInfo.id
