    ...
    >
      <Tooltip content="Detail" placement="top" arrow animation="shift-away">
        <Link
          ...
          href={`/admin/pemohon/${encodeId(rowData.id)}`}
        >
      ...
      <Tooltip content="Edit" placement="top" arrow animation="shift-away">
        <Link
          className="action-btn"
          href={`/admin/pemohon/${encodeId(rowData.id)}/edit`}
          scroll={false}
        >
...
function TablePemohon({ data = [] }) {
  ...
  const handleDelete = async (id) => {
    ...
    setDeletingRowId(id);
    try {
      const res = await axios.delete(`/api/pemohon/${id}`);
  ...
  const columns = [
    ...
    COLUMNHELPER.accessor("nomor_identitas", { header: "nomor_identitas" }),
    ...
    <Card
      ...
      headerslot={
        <div className="flex flex-col md:flex-row gap-2">
          ...
          />
          <ButtonExport data={safeData} fileName="Pemohon" />
        </div>
      }
    >
