    ...
    >
      <Tooltip content="Edit" placement="top" arrow animation="shift-away">
        <Link
          ...
          href={`/admin/peraturan/${encodeId(rowData.id)}/edit`}
          scroll={false}
        >
...
function TablePeraturan({ data = [] }) {
  ...
  const handleDelete = async (id) => {
    ...
    setDeletingRowId(id);
    try {
      const res = await axios.delete(`/api/peraturan/${id}`);
  ...
  const columns = [
    ...
    COLUMNHELPER.accessor(
      (row) => row.peraturan_header?.label || "-", // fallback null
      {
        id: "peraturan_header", // ID kolom (harus unik)
        header: "Header",
      }
    ),
    ...
    COLUMNHELPER.accessor("file", {
      ...
      cell: (info) => {
        ...
        return (
          <a
            href={`/api/services/file/uploads/${PATH_UPLOAD.peraturan}/${url}`}
            target="_blank"
