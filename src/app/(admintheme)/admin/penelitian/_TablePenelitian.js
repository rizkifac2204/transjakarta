    ...
    >
      <Tooltip content="Detail" placement="top" arrow animation="shift-away">
        <Link
          ...
          href={`/admin/penelitian/${encodeId(rowData.id)}`}
        >
      ...
      <Tooltip content="Edit" placement="top" arrow animation="shift-away">
        <Link
          className="action-btn"
          href={`/admin/penelitian/${encodeId(rowData.id)}/edit`}
        >
...
function TablePenelitian({ data = [] }) {
  ...
  const handleDelete = async (id) => {
    ...
    setDeletingRowId(id);
    try {
      const res = await axios.delete(`/api/penelitian/${id}`);
  ...
  return (
    <Card
      title="DATA PERMOHONAN PENELITIAN"
      headerslot={
        <div className="flex flex-col md:flex-row gap-2">
          ...
          />
          <ButtonExport data={safeData} fileName="Permohonan-Penelitian" />
        </div>
      }
    >
...
export default TablePenelitian;
