    ...
    >
      <Tooltip content="Edit" placement="top" arrow animation="shift-away">
        <Link
          ...
          href={`/admin/ukpbj/regulasi/${encodeId(rowData.id)}/edit`}
          scroll={false}
        >
...
function TableUkpbjRegulasi({ data = [] }) {
  ...
  const handleDelete = async (id) => {
    ...
    setDeletingRowId(id);
    try {
      const res = await axios.delete(`/api/ukpbj/regulasi/${id}`);
  ...
  const columns = [
    ...
    COLUMNHELPER.accessor(
      (row) => row.ukpbj_regulasi_header?.label || "-", // fallback null
      {
        id: "ukpbj_regulasi_header", // ID kolom (harus unik)
        header: "Header",
      }
    ),
    ...
    COLUMNHELPER.accessor("file", {
      ...
      cell: (info) => {
        ...
        return (
          <a
            href={`/api/services/file/uploads/${PATH_UPLOAD.ukpbj.regulasi}/${url}`}
            target="_blank"
...
export default TableUkpbjRegulasi;
