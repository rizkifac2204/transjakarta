...
function FormEditUkpbjRegulasi({ data, headers }) {
  ...
  async function onSubmit(formDataRaw) {
    try {
      ...
      const res = await axios.patch(
        `/api/ukpbj/regulasi/${formDataRaw.id}`,
        formData,
        {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="space-y-3">
        <Select
          ...
          placeholder={!headers ? "Memuat data..." : "Pilih Header Regulasi"}
          disabled={!headers}
          {...register("header_id")}
        />
...
export default FormEditUkpbjRegulasi;
